...
- **Planned -- fatigue should cost performance, and currently does not.** Worth stating plainly because it is easy to assume otherwise: fatigue is never read by `computeOffenseStrength`, `computeResistance`, or anything else that resolves a possession. A gassed player shoots exactly as well as a fresh one. Its only jobs today are triggering substitutions and filling the simcast's energy bars, which means the entire system is a rotation signal rather than a simulation input. The sketch: a multiplier on the `offenseMultiplier` term (where `focusPaceEfficiency` and synergy already fold in) and on resistance, scaled by fatigue above some threshold so the first two thirds of a shift cost nothing. **Deliberately a separate pass**, for two reasons -- it touches every possession and will move `scoringCalibration.test.ts`, and doing it alongside the rest system above would leave neither effect attributable. That is the same discipline that kept the position-fit retune off penalty magnitudes.
