...
export function RunProvider({ children }: { children: ReactNode }) {
  ...
  /** Shared by buyPlayerCamp/buyTeamCamp: recomputes Team.synergyScore from the camp-boosted
   *  roster's fit to the drafted system -- the same math the initial draft-time score was
   *  computed with (Team.synergyScore's doc comment flags camps as one of the systems expected to
   *  feed back into it), not a parallel invented bump. */
  const recomputeSynergyAfterCamp = useCallback((teams: RunBundle['teams'], updatedPlayers: RunBundle['players'], teamId: string) => {
  ...
  const buyPlayerCamp = useCallback(
    async (playerId: PlayerId, attribute: AttributeKey) => {
      ...
      const updatedTeams = recomputeSynergyAfterCamp(bundle.teams, updatedPlayers, bundle.run.teamId)
    ...
    [bundle, recomputeSynergyAfterCamp],
  )
